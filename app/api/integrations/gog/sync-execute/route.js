...
export async function POST() {
  ...
  if (!platformAccount || platformAccount.status !== "connected") {
    return apiError("PLATFORM_ACCOUNT_NOT_CONNECTED", "Connect your GOG account before syncing.", 400);
  }

  const syncRun = await prisma.platformSyncRun.create({
    data: { platformAccountId: platformAccount.id, provider: "gog", mode: "execute", status: "syncing", startedAt: new Date() },
  });
  ...
  try {
    ...
    return apiOk({ mode: "execute", provider: "gog", summary: { rawGamesDetected: rawGames.length, matchedCanonicalGames: matchedCount, unmatchedGames: unmatchedCount, userGamesCreated, userGamesUpdated } });
  } catch (error) {
