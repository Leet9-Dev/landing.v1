...
export async function POST() {
  ...
  try {
    ...
    const mockResolved = normalized.map((g) => ({ ...g, canonicalGameId: matchDetectedGameToCanonical("xbox", g.externalId, MOCK_EXTERNAL_SOURCES) }));
    const unmatchedForIgdb = mockResolved.filter((g) => !g.canonicalGameId).map((g) => g.externalId);
    const igdbMap = await batchMatchToIgdb("xbox", unmatchedForIgdb);
