...
export async function POST() {
  ...
  if (!platformAccount || platformAccount.status !== "connected") {
    return apiError("PLATFORM_ACCOUNT_NOT_CONNECTED", "Connect your Ubisoft Connect account before syncing.", 400);
  }
  ...
  try {
    ...
    const normalized = normalizeUbisoftGames(rawGames);
    const resolved = normalized.map((g) => ({
      ...g,
      canonicalGameId: matchDetectedGameToCanonical("ubisoft", g.externalId, MOCK_EXTERNAL_SOURCES),
    }));
    ...
    for (const g of resolved) {
      await prisma.platformDetectedGame.upsert({
        where: { platformAccountId_provider_externalGameId: { platformAccountId: platformAccount.id, provider: "ubisoft", externalGameId: g.externalId } },
        create: { platformAccountId: platformAccount.id, syncRunId: syncRun.id, provider: "ubisoft", externalGameId: g.externalId, externalTitle: g.externalTitle, playtimeHours: g.playtimeHours, lastPlayedAt: g.lastPlayedAt ? new Date(g.lastPlayedAt) : null, canonicalGameId: g.canonicalGameId, matchStatus: g.canonicalGameId ? "matched" : "unmatched", normalized: g },
    ...
    for (const g of resolved) {
      ...
      await prisma.userGame.upsert({
        ...
        create: { userId, canonicalGameId: g.canonicalGameId, sourceProvider: "ubisoft", sourcePlatformAccountId: platformAccount.id, firstDetectedAt: now, lastDetectedAt: now, playtimeHours: g.playtimeHours, sourceConfidence: "high" },
        update: { lastDetectedAt: now, sourceProvider: "ubisoft", sourcePlatformAccountId: platformAccount.id, playtimeHours: g.playtimeHours, sourceConfidence: "high" },
      });
