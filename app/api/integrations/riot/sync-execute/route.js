...
import { fetchRiotGames } from "@/lib/integrations/riot/riotClient";
...
export async function POST() {
  ...
  if (!platformAccount || platformAccount.status !== "connected") {
    return apiError("PLATFORM_ACCOUNT_NOT_CONNECTED", "Connect your Riot account before syncing.", 400);
  }

  const riotId = platformAccount.externalUserId;

  const syncRun = await prisma.platformSyncRun.create({
    data: { platformAccountId: platformAccount.id, provider: "riot", mode: "execute", status: "syncing", startedAt: new Date() },
  });
  ...
  try {
    ...
    const normalized = normalizeRiotGames(rawGames);

    const resolved = normalized.map((g) => ({
      ...g,
      canonicalGameId: matchDetectedGameToCanonical("riot", g.externalId, MOCK_EXTERNAL_SOURCES),
    }));
    ...
    for (const g of resolved) {
      await prisma.platformDetectedGame.upsert({
        where: { platformAccountId_provider_externalGameId: { platformAccountId: platformAccount.id, provider: "riot", externalGameId: g.externalId } },
        create: { platformAccountId: platformAccount.id, syncRunId: syncRun.id, provider: "riot", externalGameId: g.externalId, externalTitle: g.externalTitle, playtimeHours: null, lastPlayedAt: g.lastPlayedAt ? new Date(g.lastPlayedAt) : null, canonicalGameId: g.canonicalGameId, matchStatus: g.canonicalGameId ? "matched" : "unmatched", normalized: g },
    ...
    for (const g of resolved) {
      ...
      await prisma.userGame.upsert({
        ...
        create: { userId, canonicalGameId: g.canonicalGameId, sourceProvider: "riot", sourcePlatformAccountId: platformAccount.id, firstDetectedAt: now, lastDetectedAt: now, playtimeHours: null, sourceConfidence: "high" },
        update: { lastDetectedAt: now, sourceProvider: "riot", sourcePlatformAccountId: platformAccount.id, sourceConfidence: "high" },
      });
