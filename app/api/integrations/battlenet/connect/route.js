...
export async function GET(request) {
  ...
  if (!BATTLENET_CLIENT_ID) {
    return apiError("BATTLENET_NOT_CONFIGURED", "Battle.net integration is not configured.", 503);
  }
  ...
  headers.append("Set-Cookie", `battlenet_oauth_state=${state}; ${cookieOpts}`);
  headers.append("Set-Cookie", `battlenet_oauth_return=${encodeURIComponent(returnTo)}; ${cookieOpts}`);
