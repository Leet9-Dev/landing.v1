...
export async function POST() {
  ...
  if (!platformAccount || platformAccount.status !== "connected") {
    return apiError("PLATFORM_ACCOUNT_NOT_CONNECTED", "Connect your itch.io account before syncing.", 400);
  }

  const syncRun = await prisma.platformSyncRun.create({
    data: { platformAccountId: platformAccount.id, provider: "itch", mode: "execute", status: "syncing", startedAt: new Date() },
  });
  await prisma.platformAccount.update({ where: { id: platformAccount.id }, data: { syncStatus: "syncing" } });

  try {
    const rawGames = await fetchItchOwnedGames(platformAccount.externalUserId ?? "fixture");
    const normalized = normalizeItchGames(rawGames);
    const resolved = normalized.map((g) => ({
      ...g,
      canonicalGameId: matchDetectedGameToCanonical("itch", g.externalId, MOCK_EXTERNAL_SOURCES),
    }));
    ...
    for (const g of resolved) {
      await prisma.platformDetectedGame.upsert({
        where: { platformAccountId_provider_externalGameId: { platformAccountId: platformAccount.id, provider: "itch", externalGameId: g.externalId } },
        create: { platformAccountId: platformAccount.id, syncRunId: syncRun.id, provider: "itch", externalGameId: g.externalId, externalTitle: g.externalTitle, playtimeHours: null, lastPlayedAt: null, canonicalGameId: g.canonicalGameId, matchStatus: g.canonicalGameId ? "matched" : "unmatched", normalized: g },
    ...
    for (const g of resolved) {
      ...
      await prisma.userGame.upsert({
        ...
        create: { userId, canonicalGameId: g.canonicalGameId, sourceProvider: "itch", sourcePlatformAccountId: platformAccount.id, firstDetectedAt: now, lastDetectedAt: now, playtimeHours: null, sourceConfidence: "high" },
        update: { lastDetectedAt: now, sourceProvider: "itch", sourcePlatformAccountId: platformAccount.id, sourceConfidence: "high" },
      });
    ...
    return apiOk({ mode: "execute", provider: "itch", summary: { rawGamesDetected: rawGames.length, matchedCanonicalGames: matchedCount, unmatchedGames: unmatchedCount, userGamesCreated, userGamesUpdated } });
  } catch (error) {
