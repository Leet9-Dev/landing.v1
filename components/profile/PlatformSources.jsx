...
const CONNECT_HINT = {
  steam: {
    ...
    help: "Your public 64-bit Steam ID. Enables full game library and achievement sync.",
  },
  psn: {
    placeholder: "NPSSO token (from ca.account.sony.com)",
    help: "Paste your NPSSO token to enable full trophy sync.",
  },
};
...
export function PlatformSources() {
  ...
  useEffect(() => {
    ...
    if (connected === "1") {
      ...
      const text = DISCORD_ERROR_MESSAGES[error] || "Discord connection failed. Please try again.";
  ...
  async function connect(provider) {
    ...
    if (!value) { setNotice({ tone: "error", text: "Enter your account identifier first." }); return; }
    setBusy(provider); setNotice(null);
    try {
      const body = provider === "psn"
        ? { provider, npsso: value }
  ...
  async function sync(provider) {
    setBusy(`sync_${provider}`); setNotice(null);
    try {
      const res  = await fetch(`/api/integrations/${provider}/sync-execute`, { method: "POST" });
      const json = await res.json();
      if (json.ok) {
        const s = json.data?.summary ?? {};
        const trophyPart      = provider === "psn"   && s.trophiesDetected   > 0 ? `, ${s.trophiesDetected} trophies`   : "";
        ...
        else if (code === "PSN_SESSION_EXPIRED") setNotice({ tone: "error", text: "Your PSN session has expired. Reconnect with a new NPSSO token." });
        else setNotice({ tone: "error", text: json.error?.message || "Sync failed. Please try again." });
      }
    } catch { setNotice({ tone: "error", text: "Network error. Please try again." }); }
    setBusy(null);
  }
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      {errored && (
        <div style={{ fontSize: 12, color: "#f87171", marginBottom: 12, padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(248,113,113,0.25)", background: "rgba(248,113,113,0.05)" }}>
...
function PlatformCard({ provider, value, onChange, onConnect, onDisconnect, onSync, busy, syncing }) {
  ...
  const hasSyncApi = provider.id === "steam" || provider.id === "psn";
  ...
  const hint = CONNECT_HINT[provider.id] || { placeholder: "Account identifier", help: "" };
    ...
    <div style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", padding: 16 }}>
      ...
      {contributes.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 12 }}>
          {contributes.map((c) => (
            <span key={c} style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.04em", padding: "2px 7px", borderRadius: 5, background: "rgba(255,255,255,0.04)", color: "rgba(241,243,249,0.45)" }}>{c}</span>
          ))}
        </div>
      )}
      ...
      {isConnected ? (
        <div style={{ paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ fontSize: 10, color: "rgba(241,243,249,0.3)", marginBottom: 10 }}>
            {isDiscord
              ? `Connected ${account.connectedAt ? new Date(account.connectedAt).toLocaleDateString() : ""} · Presence active`
              : lastSynced || `Connected ${account.connectedAt ? new Date(account.connectedAt).toLocaleDateString() : ""}`}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {hasSyncApi && (
              <button
                ...
                style={{
                  ...
                  background: syncing ? "rgba(200,255,0,0.4)" : "#C8FF00", color: "#07080F",
                  cursor: (syncing || busy) ? "wait" : "pointer", fontFamily: "'Outfit', sans-serif",
                }}
              >
            ...
            <button
              ...
              style={{
                ...
                color: "#f87171", cursor: (busy || syncing) ? "wait" : "pointer", fontFamily: "'Outfit', sans-serif",
              }}
            >
        ...
        <div style={{ paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          {needsReauth && (
            <div style={{ fontSize: 11, color: "#fbbf24", marginBottom: 8, padding: "6px 10px", borderRadius: 7, border: "1px solid rgba(251,191,36,0.2)", background: "rgba(251,191,36,0.05)" }}>
              Your PSN session has expired. Enter a new NPSSO token to reconnect.
            </div>
          )}
          {isDiscord ? (
            <a
    ...
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12 }}>
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} style={{ height: 150, borderRadius: 12, background: "rgba(255,255,255,0.03)", animation: "pulse 1.4s ease infinite", animationDelay: `${i * 0.1}s` }} />
        ))}
      </div>
