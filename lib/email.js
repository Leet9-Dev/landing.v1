...
export async function sendChallengeEmail({ to, challengerName, targetProfileUrl }) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: `${challengerName} ti vuole sfidare su Leet9 — aggiungi il tuo ID`,
    html: emailHtml({
      title: "Hai una sfida in arrivo 🎮",
      body: `<strong style="color:#F1F3F9;">${challengerName}</strong> ha visitato il tuo profilo Leet9 e vuole sfidarti — ma non riesce ancora a confrontarsi con te perché non hai collegato i tuoi account di gioco.<br><br>Collega Steam, PSN o Xbox e accetta la sfida. Chi ha le stats migliori?`,
      ctaLabel: "Accetta la sfida →",
      ctaUrl: targetProfileUrl,
      footer: "Hai ricevuto questa email perché un utente Leet9 vuole sfidarti. Se non sei interessato, ignorala.",
    }),
  });
}
