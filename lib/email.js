...
export async function sendLeet9ConfirmationEmail({ to, token }) {
  const resend = getResend();
  const url = `${BASE_URL}/api/auth/confirm-leet9?token=${token}`;
  await resend.emails.send({
    from: FROM,
    to,
    ...
    html: emailHtml({
      title: "Confirm your email",
      body: "Click below to confirm your email address. This enables account recovery and security notifications on Leet9.",
      ctaLabel: "Confirm Email →",
      ctaUrl: url,
      footer: "Link expires in 24 hours. If you didn't request this, you can safely ignore this email.",
    }),
  });
}
