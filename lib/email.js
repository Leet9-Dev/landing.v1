...
export async function sendOneVsOneWelcomeEmail({ to, winnerName }) {
  const resend = getResend();
  const url = `${BASE_URL}/1v1`;
  const winnerLine = winnerName
    ? `The verdict is in: <strong style="color:#C8FF00;">${winnerName}</strong> is the better gamer.`
    : "The verdict is in — one of you is the better gamer.";
  await resend.emails.send({
    from: FROM,
    to,
    subject: "Your Leet9 battle result is saved",
    html: emailHtml({
      title: "Your score is on record.",
      body: `${winnerLine}<br><br>Unlock the full breakdown — L9 Score, achievement rate, and gaming intensity — for €1. You also get <strong style="color:#C8FF00;">500 L9 Points</strong> and full access to Leet9.`,
      ctaLabel: "See the full breakdown →",
      ctaUrl: url,
      footer: "You received this because you saved a 1v1 result on Leet9. You can unsubscribe at any time.",
    }),
  });
}
