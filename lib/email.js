...
export async function sendChallengeEmail({ to, challengerName, targetProfileUrl }) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: `${challengerName} wants to challenge you on Leet9 — add your ID`,
    html: emailHtml({
      title: "You've got a challenge incoming 🎮",
      body: `<strong style="color:#F1F3F9;">${challengerName}</strong> visited your Leet9 profile and wants to go head-to-head — but can't compare stats yet because you haven't connected your gaming accounts.<br><br>Link Steam, PSN or Xbox and accept the challenge. Who's got the better stats?`,
      ctaLabel: "Accept the challenge →",
      ctaUrl: targetProfileUrl,
      footer: "You received this email because a Leet9 player wants to challenge you. If you're not interested, just ignore it.",
    }),
  });
}
