...
export async function sendNewFollowerEmail({ to, followerName }) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: `${followerName} ti sta seguendo su Leet9`,
    html: emailHtml({
      title: "Hai un nuovo follower!",
      body: `<strong style="color:#F1F3F9;">${followerName}</strong> ha iniziato a seguirti su Leet9. Sali nella classifica e fatti notare.`,
      ctaLabel: "Vai al ranking",
      ctaUrl: url,
      footer: "Ricevi questa email perché qualcuno ti ha seguito su Leet9. Non rispondere a questa email.",
    }),
  });
}
...
export async function sendYouFollowedEmail({ to, followedName }) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: `Stai seguendo ${followedName} su Leet9`,
    html: emailHtml({
      title: `Ora segui ${followedName}`,
      body: `Hai iniziato a seguire <strong style="color:#F1F3F9;">${followedName}</strong> su Leet9. Quando sali in classifica, lo vedrai nella tua sezione Friends.`,
      ctaLabel: "Vai al ranking",
      ctaUrl: url,
      footer: "Ricevi questa email come conferma di un follow che hai effettuato su Leet9.",
    }),
  });
}
