...
import { processEventV2 } from "./engineV2.js";
...
export async function processEvent(prisma, userId, eventType, payload, idempotencyKey) {
  ...
  // 2. Streak updates for login and play events — must run before rule eligibility checks
  //    so that streak-threshold rules see the current (just-incremented) streak value.
  ...
  // 3. Load active rules for this event type.
  ...
  // 4. v2.2 dual-write — XpLedger + DailyCounter + SeasonScore (non-blocking).
  // Pass the rule IDs that v1 already awarded so v2 skips eligibility re-check
  // (avoids the race where v1 sets milestoneReached before v2 can check it).
  const awardedRuleIds = awarded.map((a) => a.ruleId);
  processEventV2(prisma, userId, eventType, payload, idempotencyKey, awardedRuleIds).catch(() => {});
...
/**
 * One-time welcome bonus awarded on account creation (500 L9 Points).
 *
 * Self-seeds the GamificationRule on first call so no manual seed step is
 * required. Idempotent via idempotencyKey — safe to call multiple times.
 */
export async function emitWelcomeEvent(prisma, userId) {
  // Ensure the rule exists in DB — idempotent upsert, cheap after first call.
  await prisma.gamificationRule.upsert({
    ...
    create: {
      ...
      description: "Welcome to Leet9! Here are 500 L9 Points to kick off your gaming identity.",
      brandedName: null,
      looped: false,
      loopFrequency: null,
      points: 500,
...
export async function emitLoginEvent(prisma, userId) {
  ...
  if (result.event) {
    ...
    if (VETERAN_MILESTONES.has(totalLoginDays)) {
      ...
      ).catch((e) => console.warn("[engine] veteran_milestone failed:", e.message));
    }
  }
...
export async function emitGameHoursSingleEvent(prisma, userId, gameId, totalHoursInGame) {
  return processEvent(
    ...
    `hours_single:${userId}:${gameId}:${totalHoursInGame}`
  );
}
