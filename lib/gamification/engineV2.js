...
export async function processEventV2(prisma, userId, eventType, payload, idempotencyKey, awardedRuleIds = null) {
  ...
  // When awardedRuleIds is provided, restrict to rules v1 already awarded so we
  // skip the eligibility re-check (avoids the milestoneReached race condition).
  const filteredRules = awardedRuleIds !== null
    ? rules.filter((r) => awardedRuleIds.includes(r.id))
    : rules;

  if (filteredRules.length === 0) return { xpAwarded: 0, spAwarded: 0, entries: [], skipped: false };
  ...
  for (const rule of filteredRules) {
    ...
    // Skip eligibility re-check when v1 already validated these rules.
...
let _activeSeasonCachedAt = 0;
const SEASON_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

async function getActiveSeason(prisma) {
  const now = Date.now();
  if (_activeSeasonId !== null && now - _activeSeasonCachedAt < SEASON_CACHE_TTL_MS) {
