...
export function validateConnectInput(body) {
  ...
  if (!isSupportedPlatform(provider)) {
    return { ok: false, error: "Unsupported platform. Supported: steam, psn, xbox, riot, battlenet, epic, gog, itch, ea, ubisoft, twitch, discord." };
  }
  ...
  if (provider === "ubisoft") {
    // Ubisoft Connect username: 3–28 chars, letters/digits/underscores/hyphens.
    if (!/^[A-Za-z0-9_-]{3,28}$/.test(rawIdentifier)) {
      return { ok: false, error: "Invalid Ubisoft Connect username (3–28 chars, letters, digits, _ or -)." };
    }
    return {
      ok: true,
      value: { provider, externalUserId: rawIdentifier, username: rawIdentifier, displayName: displayName ?? rawIdentifier },
  ...
  if (provider === "twitch") {
    // Twitch username: 4–25 chars, alphanumeric + underscores.
    if (!/^[A-Za-z0-9_]{4,25}$/.test(rawIdentifier)) {
      return { ok: false, error: "Invalid Twitch username (4–25 chars, letters, digits or _)." };
    }
    return {
      ok: true,
      value: { provider, externalUserId: rawIdentifier, username: rawIdentifier, displayName: displayName ?? rawIdentifier },
  ...
  if (provider === "discord") {
    // Discord username (new format since 2023): 2–32 chars, lowercase letters/digits/underscores/periods.
    if (!/^[a-z0-9._]{2,32}$/.test(rawIdentifier)) {
      return { ok: false, error: "Invalid Discord username (2–32 chars, lowercase letters, digits, _ or .)." };
    }
    return {
      ok: true,
      value: { provider, externalUserId: rawIdentifier, username: rawIdentifier, displayName: displayName ?? rawIdentifier },
