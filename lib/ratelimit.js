...
// 10 req/60s — 1v1 compare endpoint
export const compareRatelimit = makeRatelimiter(10, "60 s");

// 5 req/60s — auth endpoint
export const authRatelimit = makeRatelimiter(5, "60 s");

// 3 req/30d per sender→target pair — challenge nudge
export const challengeRatelimit = makeRatelimiter(3, "2592000 s");

// Generic helper used by auth.js: checkRateLimit(key, identifier)
// Falls back to { success: true } if the limiter throws (e.g. Redis unreachable).
export async function checkRateLimit(key, identifier) {
  const limiter = key === "compare" ? compareRatelimit : authRatelimit;
  try {
